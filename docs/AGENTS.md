...
- 不要引入 `jackson-datatype-jsr310` 等日期时间模块，也不要注册任何 Module
  （Java 8 日期时间支持已内置进 `jackson-databind`，`JavaTimeModule` 类在 Jackson 3 中已不存在）
...
- 不要重构已有类
- 不要输出与当前任务无关的文件
- 不要使用 Jackson 2 的 import（`com.fasterxml.jackson.databind.*`）；Jackson 3 核心类在 `tools.jackson.databind.*`，注解仍在 `com.fasterxml.jackson.annotation.*`
- 不要手动 `new ObjectMapper()`；使用 Spring Boot 自动配置的 `JsonMapper` Bean
- 不要引入 `jackson-datatype-jsr310` 等日期时间模块，也不要注册任何 Module（`JavaTimeModule` 类在 Jackson 3 中已不存在）
- 不要禁 `com.fasterxml.jackson.core:jackson-annotations`：Jackson 3 的 databind 自身依赖它
