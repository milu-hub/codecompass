...
- **超大图不画全图（有意为之，不是缺陷）**：mermaid 默认 `maxEdges = 500`，一超就抛
  `Edge limit exceeded`。**硬失败已消除**（第一层）：前端 `maxEdges: 5000` 当安全网 + 渲染前按边数
...
- **大图渲染的第一层（前端，用户要求补做）**：`DependencyGraphPane` 三处改动 ——
  ① `MERMAID_CONFIG` 显式设 `maxEdges: 5000`（默认只有 500），当**安全网**，保证两边计数口径有出入时也不硬失败；
  ② **渲染前按边数预检**：超过 `MAX_RENDERABLE_EDGES = 1000` 就不交给 mermaid，直接给可读的降级说明
  （「图太大，已跳过渲染」+ 节点/边数/上限 + 引导点类看邻域），并加「图渲染中…」占位避免大图加载时看起来空白；
  ③ **把 mermaid 的 `Edge limit exceeded` 也映射到同一句降级文案**，连同原来的「渲染失败」一起不再暴露给用户。
