/**
 * 用户自填 LLM 配置（设置抽屉的唯一数据源）。
 *
 * **存储位置：只存浏览器 localStorage（key = `cc_llm_config`），服务端不持久化。**
 * 三个硬约束（与后端请求头方案配套）：
 * 1. key 只在浏览器本地，绝不写进数据库；
 * 2. 界面上只显示掩码（`sk-***abc`），不看完整值就不渲染完整值；
 * 3. 请求时才读出来放进请求头（见 api/llm.ts），不放 URL 参数、不记日志。
 *
 * 状态是**模块级共享**的：多个组件同时读配置（顶部入口的状态提示、设置抽屉），
 * 保存/清除后彼此立刻一致，不会各持一份 ref 而对不上。
 */
...
const providerLabel = computed(() => {
  const id = config.value?.provider
  return id ? presetOf(id).label : ''
})

/** 供「测试连接」「请求头注入」直接用的一次性读取（不依赖响应式状态）。 */
